...
As an example, we use a polygon of a former sandy shore habitat from Ulkonokanhietikko, Siikajoki (Finland). This polygon was digitized from old ~1970s maps, approximating the sandy shore habitat boundaries as they were then. Many of these type of habitats, which are open, have over the recent years and decades been encroached by shrubs and trees. Monitoring the overgrowth of these habitats, which are endangered, is highly important. Here we use satellite images and NDVI index from the past 10 years to see if we can see any signs of more recent encroachment.

In this tutorial, a case study is used to show how to process (calculate NDVI), visualize, and calculate statistics for an
NDVI time series derived from HLS data over a region of interest. The tutorial also shows how to export the statistics so you will have all of the information you need for your area of interest without having to download the source data files. 
...
outDir <- c("D:\\Data\\FEO\\Pebi_HLS_NDVI\\Results")
...
We import one sandy beach polygon from Ulkonokanhietikko, Siikajoki, Finland and reproject to WGS84.

```{r}
setwd("D:\\Data\\FEO\\Pebi_HLS_NDVI")
biitsi.shp = terra::vect("D:\\Data\\FEO\\Pebi_HLS_NDVI\\Ulkonokanhietikko_3067.shp")
biitsi.shp = project(biitsi.shp, "EPSG:4326")
```
...
```{r}
leaflet() %>% 
  addPolygons(data = biitsi.shp, fill = FALSE) %>% 
...
can use the `terra` package to extract the extent of input shapefile so
we can use it to create spatial search parameters.

```{r}
roi <- terra::ext(biitsi.shp)
biitsi.bbox = as.numeric(c(roi[1], roi[3], roi[2], roi[4]))
```
...
(Note 2: this took us a lot of time to figure out how to fetch all results - it was not explained in the original NASA tutorial, which was using httr library. Luckily, by using rstac library instead, one can apply the user-friendly helper tools it has, like items_fetch())
...
Next we go through the full list of results, and filter out images that are outside growing period: January-May and October-December. Optionally, May could be included also but snow/ice could still be present in the images, so for sake of demonstrating they are left out.
...
We can also easily filter out images with too much cloud cover, e.g. more than 25%, using "eo:cloud_cover" property:
```{r}
...
Next, let's load the browse image to get a quick view of the first feature. In this case it is the first available satellite image that matches our search criteria (spatial and temporal filter), with less than 25% cloud coverage, from the region of interest in June 2013.
...
Next, convert the Shapefile projection into the native projection of HLS, WGS 84 and UTM zone X. This must be done 
in order to use the Region of Interest (ROI) to subset the satellite images. 

First we need to find out in which UTM zone the ROI is, and set that zone as the target CRS for reprojecting data from HLS collections that might be in other UTM zones. 

```{r, warning=FALSE}

if(roi[1] >= 18 && roi[1] <= 24) {
  print("The ROI is in UTM zone 34N") 
  target_utm = c("UTM Zone 34")
  target_crs = c("EPSG:32635")
  } else if(roi[1] >= 24 && roi[1] <= 30) {
    print("The ROI is in UTM zone 35N")
    target_utm = c("UTM Zone 35")
    target_crs = c("EPSG:32635")
    } else if(roi[1] >= 30 && roi[1] <= 36) {
      print("The ROI is in UTM zone 36N")
      target_utm = c("UTM Zone 36")
      target_crs = c("EPSG:32636")
      } else {
        print("The ROI is not in Finland.")
      }

#Reproject the ROI shapefile from native crs to the target CRS 
biitsi.shp.utm <- terra::project(biitsi.shp, target_crs) 
```
...
for cloud contamination using Fmask (this is not implemented yet!). 
...
**Important note: Because the HLS data possibly might be in different UTM zones, the ROI shapefile is reprojected each time to the same UTM zone as the satellite image to be cropped. ***
...
#  setTxtProgressBar(pb,row)
  if (search_df$band[row] == 'B04'){
    ...
    crop_ROI <- terra::project(biitsi.shp, crs(terra::rast(paste0('/vsicurl/',search_df$Asset_Link[row]))))
    red_crop <- terra::mask(terra::crop(red, terra::ext(crop_ROI)), crop_ROI)
    ...
    crop_ROI <- terra::project(biitsi.shp, crs(terra::rast(paste0('/vsicurl/',search_df$Asset_Link[row]))))
    fmask_crop <- terra::mask(terra::crop(fmask, terra::ext(crop_ROI)), crop_ROI)
    ...
    crop_ROI <- terra::project(biitsi.shp, crs(terra::rast(paste0('/vsicurl/',search_df$Asset_Link[row]))))
    nir_crop <- terra::mask(terra::crop(nir, terra::ext(crop_ROI)), crop_ROI)
...
Now, the cropped and stacked rasters are loaded into memory without being downloaded. 
...
Also here we have to take care of data possibly in different UTM zones. This code snippet finds out if there are any images that are in the same zone as the ROI. If yes, that image is used as the reference extent. (what image it is doesnt matter, only extent is used)
```{r}

for (i in 1:length(ndvi_stack)) {
  if(grepl(target_utm, c(crs(ndvi_stack[[i]])))) {
    ref.ext = ndvi_stack[[i]]
    } 
}
```

Next, check if the reference extent object (from code above) exists. If not, it means that all data in our NDVI stack is in different utm zone than the ROI. Then we reproject the ROI geometry to same UTM zone as the images, crop and mask.

If the reference extent object exits, then some of the images need to be reprojected. Find all data  which is not in the target UTM zone and reproject, crop, mask and resample to reference extent.
```{r}
if(!exists("ref.ext")) {
   for (i in 1:length(ndvi_stack)) {
    if(!grepl(target_utm, c(crs(ndvi_stack[[i]])))) {
      biitsi.shp.utm = project(biitsi.shp.utm, c(crs(ndvi_stack[[i]])))
      ndvi_stack[[i]] = terra::crop(ndvi_stack[[i]], terra::ext(biitsi.shp.utm))
      ndvi_stack[[i]] = terra::mask(ndvi_stack[[i]], biitsi.shp.utm)
      }
  }   
} else {
   for (i in 1:length(ndvi_stack)) {
    if(!grepl(target_utm, c(crs(ndvi_stack[[i]])))) {
      ndvi_stack[[i]] = terra::project(ndvi_stack[[i]], target_crs)
      ndvi_stack[[i]] = terra::crop(ndvi_stack[[i]], terra::ext(biitsi.shp.utm))
      ndvi_stack[[i]] = terra::mask(ndvi_stack[[i]], biitsi.shp.utm)
      ndvi_stack[[i]] = terra::resample(ndvi_stack[[i]], ref.ext)
      }
  } 
}

#Finally create a raster object from the ndvi stack.
...
Remove variables not needed anymore to save memory.
```{r}
rm(nir_stack, red_stack, fmask_stack, date_list, ndvi_stack)
```
...
the layer. 
Here we plot just the 50 first NDVI images in the time series (years 2013-2017).
...
Above, the time series show annual and inter-annual changes of NDVI.  For some images it is obvious that taking advantage of the cloud detection algorithm (fmask) could further improve the quality of the NDVI time series, which now shows in strange NDVI values. Also note that there are water pixels included (sometimes a negative buffer might help to mitigate edge effects).
